...
namespace Distance::Utils
{
    ...
    {
        data.resize(1024);
        DWORD sizeInBytes =static_cast<DWORD>(data.size());
    ...
    {
        data.resize(512);
        DWORD sizeInBytes = static_cast<DWORD>(data.size() * sizeof(wchar_t));
        LSTATUS status = RegQueryValueExW(
            subKey_,
            fieldName.data(),
            nullptr,
    ...
    ErrorCode Registry::setData(const std::wstring& data, std::wstring_view fieldName)
    {
        return RegSetValueExW(
            ...
            static_cast<DWORD>(data.size() * sizeof(wchar_t)) /* size in bytes */
        );
    }
